    ...
    <section className="pb-[20px]">
      <div className="container mt-[30px]">
        <div className="flex items-center justify-between ">
          <h2 className="font-bold text-[30px] text-primary">My Watched</h2>
          <div className="bg-[#4CACBC] text-white px-[15px] py-[5px] rounded-[30px]">{watched.length} {watched.length == 1 ? "Movie" : "Movies"}</div>
        </div>
        <div className="flex gap-[23px] mt-[20px] flex-wrap ">
            {watched?.map((poster,key)=> (
              ...
              onMouseEnter ={(e)=>handleMouseEnter(e)} onMouseLeave={(e)=>handleMouseLeave(e)}>
                ...
                <div className="absolute z-10 text-white bottom-[30px] left-[50%]
                  translate-x-[-50%] text-[25px] hidden">
                    ...
                    onClick={()=>MovieDispatch({type : actions.REMOVE_MOVIE_FROM_WATCHED, pyload : poster})}><i className="fa-solid fa-xmark"></i></span>
                    <span className="cursor-pointer hover:text-green-500"
                    onClick={()=>MovieDispatch({type : actions.MOVE_TO_WATCHLIST, pyload : poster})}><i className="fa-regular fa-eye-slash"></i></span>
                </div>
              </div>
            ))}
            {watched.length == 0 ? 
